...
def get_contours_from_mask(
    mask: np.ndarray, min_area: int = 5
) -> list[np.ndarray]:
...
def get_larger_contour(contours: list[np.ndarray]) -> np.ndarray | None:
    larger_area = 0
    larger_contour = None
    for contour in contours:
...
def black2white(image: np.ndarray) -> np.ndarray:
    lo = np.array([0, 0, 0])
    hi = np.array([0, 0, 0])
...
def process_image(
    img: np.ndarray, n_w: int = 256, n_h: int = 64
) -> np.ndarray:
    w, h, _ = img.shape
    new_w = n_h
    new_h = int(h * (new_w / w))
...
def crop_img_by_polygon(img: np.ndarray, polygon: np.ndarray) -> np.ndarray:
